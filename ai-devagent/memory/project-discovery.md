...
**Purpose**: Structured discovery sequence for building project memory in a repository that has
no `.github/ai-memory/` yet, without broad repository scans.

**When to use**: First time working in a repository that has no `.github/ai-memory/` yet.

The AI should NEVER start by scanning the repository. Use this sequence to build project memory efficiently.
